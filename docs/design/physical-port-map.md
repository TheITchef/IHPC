...
**Status:** In Revision · **Version:** 1.2 · **Last updated:** 2026-07-31 · **Owner:** Ioannis Mintzivyris
...
The out-of-band switch is where the management segment connects. The segment itself extends to the core switch, which routes it. This switch exists so that baseboard management controllers (BMCs) — iDRAC on Dell hardware, iLO on HPE — remain reachable when the production path is unavailable. It also carries the privileged access workstation from which all network devices and controllers are administered.
...
itc-uvy-esxi02 carries four onboard interfaces only, with no PCIe card fitted. None are cabled; only its iDRAC is connected. Its role remains undecided and will be recorded in an Architectural Decision Record when Phase 3 begins.

Patch panel ports 05–09, 11, 12, 16, 17, 19, 20 and 22 are unused on both sides.

## 7. Acceptance criteria

- Every cabled link is recorded consistently wherever it appears: in the patch panel table if patched, in the direct runs table if it bypasses the panel, and in the port table of each device it terminates on.
